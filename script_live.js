        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                setTimeout(async () => {
                    ...
                    const savedQuote = sessionStorage.getItem('pendingQuote') || localStorage.getItem('pendingQuote');
                    if (savedQuote) {
                ...
                if (window.isPhoneVerified) {
                    ...
                    if (phoneInput) { 
                        ...
                        if (memberPhone) {
                            ...
                            phoneInput.placeholder = "연락처를 입력해주세요";
                        }
                    }
                    window.isPhoneVerified = true;
                } else {
...
`.trim();
                    ...
                    // Set expected price in auth panel area (optional, can add later)
                    
                    // --- 패널 열기 + 수거일자 채우기 ---
                    const authPanel = document.getElementById('auth-delivery-panel');
                    if (authPanel) {
                        // 수거일자 셀렉트 채우기
                        const authPickupSelect = document.getElementById('auth-courier-pickup-date');
                        if (authPickupSelect && authPickupSelect.options.length <= 1) {
                            authPickupSelect.innerHTML = '<option value=\"\">방문 날짜를 선택해주세요</option>';
                            ...
                            for (let i = 1; i <= 7; i++) {
                                ...
                                authPickupSelect.appendChild(opt);
                            }
                        }

                        // 패널 열기 애니메이션
                        authPanel.style.maxHeight = '2000px';
                        authPanel.style.opacity = '1';
                        authPanel.style.padding = '20px';

                        // Auth 버튼 숨기기
                        const authBtnArea = btnAuthNext.closest('.text-center');
                    ...
                    btnAuthNext.textContent = '다음 단계로 →';
        ...
        // --- Auth 패널 내 배송방법 토글 ---
        const authDeliveryPanel = document.getElementById('auth-delivery-panel');
        if (authDeliveryPanel) {
            authDeliveryPanel.querySelectorAll('.method-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    authDeliveryPanel.querySelectorAll('.method-btn').forEach(b => b.classList.remove('active'));
                    btn.classList.add('active');
                    const method = btn.dataset.method;
                    const cvsInfo = document.getElementById('auth-method-cvs-info');
                    const courierDate = document.getElementById('auth-method-courier-date');
                    const addrSection = document.getElementById('auth-address-section');
                    const addrLabel = document.getElementById('auth-address-label');
                    if (cvsInfo) cvsInfo.style.display = method === 'cvs' ? 'block' : 'none';
                    if (courierDate) courierDate.style.display = method === 'courier' ? 'block' : 'none';
                    if (addrSection) addrSection.style.display = method === 'cvs' ? 'none' : 'block';
                    if (addrLabel) addrLabel.style.display = method === 'cvs' ? 'none' : 'flex';
                });
            });
        }

        // --- Auth 패널 "접수 완료하기" 버튼 핸들러 ---
        const btnAuthDeliverySubmit = document.getElementById('btn-auth-delivery-submit');
        if (btnAuthDeliverySubmit) {
            btnAuthDeliverySubmit.addEventListener('click', async () => {
                ...
                const activeBtn = authDeliveryPanel.querySelector('.method-btn.active');
                if (activeBtn) deliveryMethod = activeBtn.dataset.method;

                // 수거일자
                let pickupDate = '';
                if (deliveryMethod === 'courier') {
                    const pickupElem = document.getElementById('auth-courier-pickup-date');
                    if (pickupElem) pickupDate = pickupElem.value;
                }

                // 주소
                const baseAddress = document.getElementById('auth-customer-address').value.trim();
                const detailAddress = document.getElementById('auth-customer-address-detail') ? document.getElementById('auth-customer-address-detail').value.trim() : '';
                const address = detailAddress ? `${baseAddress} ${detailAddress}` : baseAddress;

                // 은행/계좌
                const bankSelect = document.getElementById('auth-customer-bank');
                let bankName = '';
                if (bankSelect) {
                    if (bankSelect.value === 'direct') {
                        const directInput = document.getElementById('auth-bank-direct-text');
                        bankName = directInput ? directInput.value.trim() : '';
                    } else {
                        bankName = bankSelect.value;
                    }
                }
                const accountNum = document.getElementById('auth-customer-account') ? document.getElementById('auth-customer-account').value.trim() : '';
                const account = bankName ? `${bankName} ${accountNum}` : accountNum;
                
                // 메모
                const memo = document.getElementById('auth-customer-memo') ? document.getElementById('auth-customer-memo').value.trim() : '';

                // 유효성 검사
                const errorMsg = document.getElementById('auth-delivery-error-msg');
                ...
                if (needsAddress && !baseAddress) {
                    if (errorMsg) {
                        errorMsg.innerText = "주소 검색 버튼을 눌러 수거 주소를 입력해주세요.";
                        errorMsg.style.display = 'block';
                    }
                    return;
                }
                if (!bankName || bankName === 'direct') {
                    if (errorMsg) {
                        errorMsg.innerText = "은행을 선택해주세요.";
                        errorMsg.style.display = 'block';
                    }
                    return;
                }
                if (!accountNum) {
                    if (errorMsg) {
                        errorMsg.innerText = "계좌번호를 입력해주세요.";
                        errorMsg.style.display = 'block';
                    }
                    return;
                }

                btnAuthDeliverySubmit.textContent = '처리 중...';
                btnAuthDeliverySubmit.disabled = true;
...
`.trim();
                    ...
                    if (window.karrotPixel) {
                        try { window.karrotPixel.track('SubmitApplication'); } catch(e) {}
                    }
                    ...
                    if (typeof gtag === 'function') {
                        try {
                            ...
                            const fd = snap2.exists() ? snap2.data() : {};
                            gtag('event', 'quote_completed', {
                                'event_category': 'Quote',
                                'event_label': fd.model || '',
                                'value': fd.price || 0
                            });
                        } catch(e) {}
                    }
                    ...
                    // --- UI 전환: 완료 화면 ---
                    // Auth 전체 숨기고 Step 8 완료 화면 보여주기
                    document.getElementById('wizard-step-auth').style.display = 'none';
                    const step8 = document.getElementById('wizard-step-8');
                    if (step8) {
                        step8.style.display = 'block';
                        const deliverySection = document.getElementById('step8-delivery-section');
                    ...
                    // Show modal after UI transition
                    window.executeFinalSubmit = () => {}; // Prevent re-submission when modal closes
                    if (window.openPresaleModal) window.openPresaleModal();

                } catch (e) {
                    console.error("배송 접수 완료 오류:", e);
                    alert("저장 중 오류가 발생했습니다.");
                    btnAuthDeliverySubmit.textContent = '접수 완료하기 ✅';
                    btnAuthDeliverySubmit.disabled = false;
                }
            });
        }

        // Delivery Method Listeners (Step 8 - resume flow용)
        const deliveryBtns = document.querySelectorAll('#wizard-step-8 .method-btn');
        console.log("Found delivery buttons:", deliveryBtns.length);

        deliveryBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('#wizard-step-8 .method-btn').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                ...
                const courierDate = document.getElementById('method-courier-date');
    ...
    if (btnSubmitDelivery) {
        btnSubmitDelivery.addEventListener('click', async () => {
            ...
            if (bankSelect && bankSelect.tagName === 'SELECT') {
                if (bankSelect.value === 'direct') {
                    const directInput = document.getElementById('step8-bank-direct-text');
                    bankName = directInput ? directInput.value.trim() : '';
                } else {
                    bankName = bankSelect.value;
                }
            ...
            let deliveryMethod = 'courier';
            const activeBtn = document.querySelector('#wizard-step-8 .method-btn.active');
            if (activeBtn) deliveryMethod = activeBtn.dataset.method;
