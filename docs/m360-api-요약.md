...
## 3-1. ⚠️ 어디까지 자동으로 넣을지 — 사장님이 정한 범위 (2026-08-27)

**이 결정을 임의로 넓히지 마라.** 레코드에 값이 다 온다는 이유로
자동 입력을 늘리고 싶어지는데, 아래는 그렇게 하지 않기로 정한 것이다.

| 화면 | 자동으로 넣는 것 | 손으로 쓰는 것 |
|---|---|---|
| **매매계약서** | **IMEI 뿐** | 그 외 **전부** — 하자·상태·등급 모두 검수원이 직접 |
| 고객 검수서 | 없음 | 용량도 넣지 않는다 |
| 재고 등록 | IMEI · 용량 | 나머지 |

이유: 하자 판단은 매입가를 정하는 일이다. M360 이 틀릴 수 있고,
틀린 채로 자동 체크되면 사람이 확인하지 않고 넘어간다.
M360 값은 **보여주기만** 하고, 누르는 것은 사람이 한다.

용량은 재고 관리용이다. M360 이 실물 용량(예: 512GB)을 알아내지만
고객이 신청할 때 고른 용량(예: 「기본(용량무관)」)과 다를 수 있고,
고객 검수서에 자동으로 다른 값이 찍히면 분쟁이 된다.

## 4. 나중에 쓸 필드 (레코드에 이미 다 온다 — 총 90개)

오늘 늘린 검수 하자 항목과 그대로 이어진다.
**보여주는 용도로만 쓴다 — 위 3-1 의 범위를 지킬 것. 자동으로 체크하지 마라.**
