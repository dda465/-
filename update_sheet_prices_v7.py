...
def get_update_rule(brand_raw, series_raw, model_raw):
    ...
    if "애플" in brand or "APPLE" in brand:
        return {'type': 'mul', 'val': 0.97, 'label': '-3%'}
    return {'type': 'none'}
...
def main():
    ...
    with open('price_decrease_report_v7.md', 'w', encoding='utf-8') as out:
        out.write("# 📱 7차 단가 하향 내역 (S급 기준)\n\n")
        out.write("요청하신 대로 **애플 아이폰 전 기종 3% 하향 (아이폰 17 포함)**을 적용하여 전체 단가를 업데이트하였습니다.\n")
